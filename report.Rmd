---
title: "Tuning Neural Networks with R"
...
A general rule of thumb that I like to apply in order to decide to decide the number of _total_ neurons in the hidden layers, is this following formula:
...
* $\alpha$ is an arbitrary scaling factor between 1 and 10.
...
In order to measure it, we'll just train a simple Neural Network usuing the `h2o` package and use it for different levels of complexity. We are going to evaluate the performance of our model in a validation set and we'll write down how much time it took every time.
